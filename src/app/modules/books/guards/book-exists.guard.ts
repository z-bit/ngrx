...
export class BookExistsGuard implements CanActivate {
    ...
    waitForCollectionToLoad(): Observable<boolean> {
        return this.store
            .select(fromRoot.getCollectionLoaded)
    ...
    hasBookInStore(id: string): Observable<boolean> {
        return this.store
            .select(fromRoot.getBookEntities)
    ...
    hasBookInApi(id: string): Observable<boolean> {
        return this.googleBooksService
            ...
            .map( book => !!book )
            .catch( () => {
                this.router.navigate(['/books/404']);
                return of(false);
            })
        ;
    }
    ...
    /**
     * This is the actual method the router will call when the guard is run.
     *
     * The guard waits for the collection to load, then it checks if we need
